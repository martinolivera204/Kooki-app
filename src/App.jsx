    ...
    <div style={{ minHeight:"100vh", background:C2.white, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 24px", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ width:"100%", maxWidth:380, animation:"fadeIn 0.4s ease" }}>
        ...
        <div style={{ background:C2.white, borderRadius:24, padding:"32px 28px", boxShadow:"0 8px 40px rgba(59,111,212,0.12)", border:`1px solid ${C2.gray2}` }}>
          ...
          <button onClick={handleSubmit} disabled={loading || !code.trim()} style={{ width:"100%", padding:"18px", borderRadius:16, border:"none", background:loading||!code.trim()?C2.gray2:`linear-gradient(135deg,${C2.blue},${C2.blueDk})`, color:loading||!code.trim()?C2.gray4:C2.white, fontSize:17, fontWeight:800, cursor:loading||!code.trim()?"not-allowed":"pointer", fontFamily:"'DM Sans',sans-serif", boxShadow:!loading&&code.trim()?"0 8px 28px rgba(59,111,212,0.38)":"none" }}>
...
/* POOLS: banco de recetas por objetivo para rotacion aleatoria */
const POOLS = {
...
function generarHoy(cuantas, tipo, ingredientes) {
  ...
  // Si hay ingredientes, priorizar recetas que los usen
      ...
      <div style={{ position:"relative", background:C.white, borderRadius:"24px 24px 0 0", width:"100%", maxWidth:580, margin:"0 auto", maxHeight:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 -8px 40px rgba(59,111,212,0.2)" }}>
        ...
        <div style={{ padding:"12px 16px 20px", borderTop:`1px solid ${C.gray2}`, display:"flex", gap:10, alignItems:"flex-end" }}>
          <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder="Preguntale al chef..." rows={1}
            style={{ flex:1, padding:"12px 14px", borderRadius:14, border:`2px solid ${C.gray2}`, fontSize:14, color:C.text, resize:"none", lineHeight:1.5, background:C.gray1, fontFamily:"'DM Sans',sans-serif", outline:"none", maxHeight:80, overflowY:"auto", transition:"border 0.2s" }}
    ...
    <div style={{ minHeight:"100vh", background:C.white, fontFamily:"'DM Sans',sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      {!hoyResult ? (
        <div style={{ flex:1, padding:"28px 24px", maxWidth:520, margin:"0 auto", width:"100%", overflowY:"auto" }}>
          <div style={{ fontSize:26, fontWeight:800, color:C.dark, marginBottom:6, letterSpacing:"-0.5px" }}>Rapido y sin vuelta 🍳</div>
          ...
          <div style={{ marginBottom:24 }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              {[["1","🍽️ Una sola"],["2","🌞🌙 Almuerzo y cena"]].map(([val,lbl]) => (
                <button key={val} onClick={() => setHoyCuantas(val)} style={{ flex:1, padding:"14px 10px", borderRadius:14, border:`2px solid ${hoyCuantas===val?C.blue:C.gray2}`, background:hoyCuantas===val?C.blueLt:C.white, fontSize:14, fontWeight:700, color:hoyCuantas===val?C.blueDk:C.dark, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.18s" }}>{lbl}</button>
              ))}
            </div>
          </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {[["cualquiera","🍽️","Cualquier cosa"],["liviana","🥗","Liviana y sana"],["rapida","⚡","Rapida (menos de 20 min)"],["contundente","💪","Contundente"],["vegetariana","🌿","Vegetariana / Vegana"]].map(([val,emoji,lbl]) => (
                <button key={val} onClick={() => setHoyTipo(val)}
                  style={{ padding:"14px 10px", borderRadius:14, border:`2px solid ${hoyTipo===val?C.blue:C.gray2}`, background:hoyTipo===val?C.blueLt:C.white, fontSize:13, fontWeight:700, color:hoyTipo===val?C.blueDk:C.dark, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.18s", display:"flex", flexDirection:"column", alignItems:"center", gap:6, gridColumn:val==="vegetariana"?"span 2":"span 1" }}>
          ...
          <div style={{ marginBottom:32 }}>
            ...
            <textarea value={hoyIngredientes} onChange={e => setHoyIngredientes(e.target.value)} placeholder="Ej: pollo, arroz, huevos, tomates..." rows={2}
              style={{ width:"100%", padding:"14px 16px", borderRadius:14, border:`2px solid ${C.gray2}`, fontSize:15, color:C.text, resize:"none", lineHeight:1.6, background:C.gray1, fontFamily:"'DM Sans',sans-serif", outline:"none" }}
              onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.gray2}/>
          </div>

          <button onClick={() => setHoyResult(generarHoy(hoyCuantas, hoyTipo, hoyIngredientes))} style={{ width:"100%", background:`linear-gradient(135deg,${C.blue},${C.blueDk})`, color:C.white, border:"none", borderRadius:18, padding:"20px", fontSize:18, fontWeight:800, cursor:"pointer", boxShadow:sh.blue, fontFamily:"'DM Sans',sans-serif" }}>
            ✨ Sugerir receta →
          </button>
        </div>
      ) : (
        <div style={{ flex:1, padding:"28px 24px", maxWidth:520, margin:"0 auto", width:"100%" }}>
            ...
            {hoyResult.map((item,i) => {
              ...
              return (
                <div key={i} style={{ background:C.white, borderRadius:18, padding:"20px", boxShadow:sh.md }}>
                  <div style={{ fontSize:12, fontWeight:800, color:C.blue, textTransform:"uppercase", letterSpacing:"1px", marginBottom:10 }}>{item.tipo}</div>
          ...
          <button onClick={() => setHoyResult(generarHoy(hoyCuantas, hoyTipo, hoyIngredientes))} style={{ width:"100%", background:C.white, color:C.blue, border:`2px solid ${C.blue}`, borderRadius:16, padding:"16px", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginBottom:12 }}>↺ Otra sugerencia</button>
          <button onClick={() => setHoyResult(null)} style={{ width:"100%", background:C.gray1, color:C.sub, border:"none", borderRadius:16, padding:"14px", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>← Volver</button>
        </div>
      )}
          ...
          {cur.type === "grid" && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {cur.options.map(opt => {
                ...
                return (
                  <button key={opt.value} onClick={toggle} style={{ padding:"18px 12px", borderRadius:16, cursor:"pointer", textAlign:"center", border:`2px solid ${sel?C.blue:C.gray2}`, background:sel?C.blueLt:C.white, display:"flex", flexDirection:"column", alignItems:"center", gap:8, transition:"all 0.18s", fontFamily:"'DM Sans',sans-serif", boxShadow:sel?`0 0 0 3px ${C.blue}22,${sh.md}`:sh.sm }}>
  ...
  if (screen === "loading") {
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.white, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32, fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <h2 style={{ fontSize:26, fontWeight:800, color:C.dark, marginBottom:10, textAlign:"center" }}>Kooki esta trabajando...</h2>
      ...
      <div ref={scrollRef} style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", overflowY:"auto" }}>
        ...
        <div style={{ background:`linear-gradient(140deg,${C.blue},${C.blueDk})`, padding:"22px 22px 36px", position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ position:"relative" }}>
            ...
            <div style={{ fontSize:24, fontWeight:800, color:C.white, marginBottom:8 }}>Tu plan esta listo 🎉</div>
