...
function AccessScreen({ onAccess }) {
  ...
  const C2 = { blue:"#3B6FD4", blueDk:"#2A52A8", blueLt:"#EEF3FC", white:"#FFFFFF", gray1:"#F0F3FA", gray2:"#E2E8F5", gray4:"#7A8BAD", dark:"#111827", text:"#1E2D4A", sub:"#5A6E8C", danger:"#EF4444", dangerLt:"#FEF2F2" };
  return (
    <div style={{ minHeight:"100vh", background:C2.white, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 24px", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ width:"100%", maxWidth:380, animation:"fadeIn 0.4s ease" }}>
        <div style={{ textAlign:"center", marginBottom:40 }}>
          <img src="https://i.imgur.com/IYcv1Vp.jpeg" alt="Kooki" style={{ width:160, borderRadius:0, marginBottom:8, background:"transparent" }}/>
        ...
        <div style={{ background:C2.white, borderRadius:24, padding:"32px 28px", boxShadow:"0 8px 40px rgba(59,111,212,0.12)", border:`1px solid ${C2.gray2}` }}>
          <div style={{ fontSize:22, fontWeight:800, color:C2.dark, marginBottom:8 }}>Ingresa tu codigo</div>
          ...
          <div style={{ marginBottom:16, animation:shake?"shake 0.5s ease":"none" }}>
            <input type="text" value={code} onChange={e => { setCode(e.target.value.toUpperCase()); setError(""); }} onKeyDown={e => e.key === "Enter" && handleSubmit()} placeholder="KOOKI-XXXX-XXXX-XXXX"
              style={{ width:"100%", padding:"16px", borderRadius:14, fontSize:16, fontWeight:600, border:`2px solid ${error ? C2.danger : code ? C2.blue : C2.gray2}`, background:error ? C2.dangerLt : C2.gray1, color:C2.dark, fontFamily:"'DM Sans',sans-serif", letterSpacing:"1px", textAlign:"center", outline:"none" }}/>
            {error && <div style={{ fontSize:13, color:C2.danger, marginTop:8, textAlign:"center", fontWeight:600 }}>⚠️ {error}</div>}
          </div>
          <button onClick={handleSubmit} disabled={loading || !code.trim()} style={{ width:"100%", padding:"18px", borderRadius:16, border:"none", background:loading || !code.trim() ? C2.gray2 : `linear-gradient(135deg,${C2.blue},${C2.blueDk})`, color:loading || !code.trim() ? C2.gray4 : C2.white, fontSize:17, fontWeight:800, cursor:loading || !code.trim() ? "not-allowed" : "pointer", fontFamily:"'DM Sans',sans-serif" }}>
...
const C = { blue:"#3B6FD4", blueDk:"#2A52A8", blueMd:"#5B8AE8", blueLt:"#EEF3FC", bluePl:"#D6E4FA", white:"#FFFFFF", bg:"#F4F7FF", gray1:"#F0F3FA", gray2:"#E2E8F5", gray3:"#B8C5DE", gray4:"#7A8BAD", dark:"#111827", text:"#1E2D4A", sub:"#5A6E8C", success:"#10B981" };
...
function KookiLogo({ size=28, dark=false }) {
  return <span style={{ fontWeight:800, fontSize:size, color:dark?"#FFF":C.blue, letterSpacing:"-1px", lineHeight:1, fontFamily:"'Nunito','DM Sans',sans-serif" }}>Kooki</span>;
}
...
const R = {
  ...
  "Tortilla de claras y vegetales":{t:"15 min",d:"Facil",k:"200 kcal",p:"24g",e:"🥚",tags:["🔥 Bajo en calorias"],i:["4 claras + 1 huevo","1 morron","½ cebolla","100g espinaca","Sal y pimienta"],s:["Saltear morron y cebolla picados 5 min.","Agregar espinaca hasta marchitar.","Batir claras y huevo con sal. Volcar.","Tapar, fuego bajo 4 min. Dar vuelta 2 min."]},
  ...
  "Sopa minestrone liviana":{t:"35 min",d:"Facil",k:"220 kcal",p:"10g",e:"🍲",tags:["🔥 Bajo en calorias"],i:["1 cebolla","2 zanahorias","2 papas","1 zapallito","½ taza fideos chicos","1 lata tomates","Caldo","Albahaca"],s:["Rehogar cebolla 4 min. Agregar zanahoria y papas.","Sumar tomates y caldo. Cocinar 15 min.","Agregar zapallito y fideos. Cocinar 10 min.","Condimentar con albahaca y sal."]},
  ...
  "Milanesas con pure de papas":{t:"30 min",d:"Facil",k:"580 kcal",p:"36g",e:"🥩",tags:["💰 Economico"],i:["4 milanesas de ternera","1 huevo","Pan rallado","4 papas","100ml leche","30g manteca","Sal, ajo y perejil"],s:["Hervir papas hasta tiernas. Pisar con leche y manteca.","Pasar milanesas por huevo batido y pan rallado.","Freir en aceite caliente 3-4 min por lado.","Servir con el pure al lado."]},
  ...
  "Salmon con quinoa y vegetales":{t:"30 min",d:"Media",k:"540 kcal",p:"46g",e:"🐟",tags:["💚 Desinflamatorio","💪 Alta proteina"],i:["200g salmon","½ taza quinoa","Brocoli y zanahoria","Limon","Eneldo o perejil","Aceite","Sal"],s:["Cocinar quinoa en agua con sal 15 min.","Cocinar vegetales al vapor 6 min.","Condimentar salmon con limon y sal. Grillar 3-4 min por lado.","Servir con quinoa y vegetales."]},
  ...
  "Fideos integrales con pesto":{t:"20 min",d:"Facil",k:"480 kcal",p:"18g",e:"🍝",tags:["🌿 Vegetariana"],i:["300g fideos integrales","Albahaca fresca 1 atado","Ajo 2 dientes","Parmesano 50g","Aceite de oliva 4 cdas","Sal y pimienta"],s:["Hervir los fideos al dente.","Procesar albahaca con ajo, parmesano y aceite hasta pasta.","Condimentar con sal y pimienta.","Mezclar fideos escurridos con el pesto."]},
  "Sopa de arvejas con jamon":{t:"30 min",d:"Facil",k:"320 kcal",p:"20g",e:"🥣",tags:["💰 Economico"],i:["400g arvejas congeladas","150g jamon cocido","1 cebolla","Caldo en cubo","Menta fresca","Sal y pimienta"],s:["Rehogar cebolla 5 min en olla con aceite.","Agregar arvejas y caldo. Cocinar 15 min.","Procesar la mitad de la sopa para darle cuerpo.","Agregar jamon en cubos y menta. Calentar 2 min."]},
  "Bowl de boniato y garbanzos":{t:"35 min",d:"Facil",k:"420 kcal",p:"16g",e:"🍠",tags:["🌿 Vegetariana","💚 Desinflamatorio"],i:["2 boniatos medianos","1 lata garbanzos","Espinaca baby","Palta","Tahini 2 cdas","Limon","Curcuma y sal"],s:["Cortar boniatos en cubos y asar 25 min a 200C con curcuma.","Escurrir y tostar garbanzos en sarten con aceite 10 min.","Armar el bowl con espinaca de base, boniato, garbanzos y palta.","Mezclar tahini con limon y agua para la salsa."]},
  ...
  "Pollo a la crema con arroz":{t:"30 min",d:"Facil",k:"510 kcal",p:"40g",e:"🍗",tags:["💪 Alta proteina"],i:["2 pechugas","Crema light 200ml","1 cebolla","Champinones 150g","Ajo","Sal, pimienta y tomillo","Arroz 1 taza"],s:["Cocinar el arroz con sal.","Saltear cebolla y ajo. Agregar pechuga en cubos.","Dorar el pollo. Agregar champinones 3 min.","Verter la crema y tomillo. Cocinar 5 min hasta que espese."]},
  "Tallarines con atun y tomate cherry":{t:"15 min",d:"Facil",k:"450 kcal",p:"28g",e:"🍝",tags:["⚡ Express","💪 Alta proteina"],i:["300g tallarines","2 latas atun","200g tomate cherry","Ajo 2 dientes","Aceite de oliva","Albahaca","Sal y pimienta"],s:["Hervir los tallarines al dente.","Calentar aceite con ajo laminado 1 min.","Agregar tomates cherry y cocinar 3 min.","Incorporar el atun y mezclar. Sumar pasta y albahaca."]},
  "Estofado de pollo con ciruelas":{t:"50 min",d:"Media",k:"480 kcal",p:"38g",e:"🍲",tags:["💰 Economico"],i:["4 presas de pollo","12 ciruelas secas","1 cebolla","1 morron","Vino blanco 100ml","Caldo","Tomillo y laurel"],s:["Dorar las presas de pollo por todos lados. Retirar.","Rehogar cebolla y morron 5 min.","Agregar el vino y dejar evaporar 2 min.","Volver el pollo con ciruelas, caldo y hierbas. Cocinar tapado 30 min."]},
  "Espinaca con ajo y huevo pochado":{t:"15 min",d:"Facil",k:"220 kcal",p:"18g",e:"🥬",tags:["🔥 Bajo en calorias","⚡ Express"],i:["400g espinaca fresca","2 huevos","3 dientes ajo","Aceite de oliva","Sal y pimienta","Pan integral tostado"],s:["Calentar aceite con ajo laminado hasta dorar.","Agregar espinaca y saltear 3 min hasta marchitar.","Pochar los huevos en agua con vinagre 3 min.","Servir la espinaca con el huevo pochado encima."]},
  "Berenjenas rellenas de carne":{t:"55 min",d:"Media",k:"430 kcal",p:"30g",e:"🍆",tags:["💰 Economico"],i:["2 berenjenas grandes","300g carne picada","1 cebolla","2 tomates","Queso rallado","Ajo, oregano y sal"],s:["Cortar berenjenas al medio y vaciar la pulpa. Horno 15 min a 180C.","Rehogar cebolla con ajo. Agregar carne y dorar.","Sumar tomate y pulpa de berenjena. Cocinar 10 min.","Rellenar las berenjenas, cubrir con queso. Hornear 20 min mas."]},
  "Risotto de champinones":{t:"35 min",d:"Media",k:"480 kcal",p:"14g",e:"🍚",tags:["🌿 Vegetariana"],i:["1.5 tazas arroz arborio","400g champinones","1 cebolla","Vino blanco 100ml","Caldo caliente 1L","Parmesano 50g","Manteca"],s:["Rehogar cebolla. Agregar arroz y tostar 2 min.","Agregar el vino y dejar evaporar.","Agregar caldo de a cucharones, esperando que se absorba cada vez.","Saltear los champinones aparte. Al final incorporar con manteca y parmesano."]},
  "Zapallitos rellenos":{t:"45 min",d:"Facil",k:"350 kcal",p:"25g",e:"🥬",tags:["💰 Economico"],i:["4 zapallitos grandes","200g carne picada o ricota","1 cebolla","1 huevo","Queso rallado","Sal y pimienta"],s:["Vaciar los zapallitos y blanquear 5 min en agua hirviendo.","Rehogar cebolla con la pulpa de zapallito.","Mezclar con proteina elegida, huevo y condimentos.","Rellenar, cubrir con queso. Hornear 25 min a 180C."]},
...
const POOLS = {
  bajar_peso:{tag:"Deficit calorico",tip:"Proteinas magras y vegetales de alto volumen. Cada comida ronda las 200-450 kcal.",precio:{bajo:"$35.000-45.000",medio:"$50.000-65.000",alto:"$70.000-90.000"},
    almuerzos:["Ensalada de pollo grillado y quinoa","Pechuga al horno con vegetales asados","Bowl de arroz integral con pollo","Pechuga al limon con quinoa y brocoli","Wrap de atun con lechuga y tomate","Ensalada completa con atun","Ensalada de garbanzos con pepino","Pollo al horno con ensalada verde","Ensalada tibia de pollo y papa","Salmon con quinoa y vegetales","Espinaca con ajo y huevo pochado"],
    cenas:["Sopa de verduras con lentejas","Revuelto de claras con espinaca","Sopa crema de zapallo","Omelette de claras con champinones","Sopa minestrone liviana","Tortilla de claras y vegetales","Sopa de tomate con albahaca","Ensalada caprese con rucula","Sopa de arvejas y menta","Creme de zanahoria y jengibre"]},
  saludable:{tag:"Nutricion completa",tip:"Plan balanceado con todos los macronutrientes. Variedad de colores = variedad de nutrientes.",precio:{bajo:"$40.000-55.000",medio:"$60.000-80.000",alto:"$90.000-120.000"},
    ...
    cenas:["Sopa de verduras con lentejas","Tortilla de papas y cebolla","Sopa crema de zapallo","Ensalada de garbanzos con pepino","Revuelto de claras con espinaca","Sopa de tomate con albahaca","Ensalada caprese con rucula","Sopa minestrone liviana","Sopa de arvejas y menta"]},
...
function generarHoy(cuantas) {
  const todas = Object.keys(R);
  const shuffled = shuffle(todas);
  if (cuantas === "1") return [{ tipo:"Tu comida de hoy", nombre:shuffled[0] }];
  return [{ tipo:"Almuerzo", nombre:shuffled[0] }, { tipo:"Cena", nombre:shuffled[1] }];
}

function Tag({ children, color="#3B6FD4", bg="#EEF3FC" }) {
...
const TAG_COLORS = {
  ...
  "⚡ Rendidor":        { bg:"#EFF6FF", color:"#3B6FD4" },
...
function MealTag({ label }) {
  const s = TAG_COLORS[label] || { bg:"#EEF3FC", color:"#3B6FD4" };
...
function ChefChat({ result, recetaActual, onClose }) {
  ...
  const responder = (p) => {
    p = p.toLowerCase();
    if (p.includes("reemplazar") || p.includes("sustituir") || p.includes("falta") || p.includes("sin ")) {
      if (p.includes("quinoa")) return "Si no tenes quinoa, podes usar arroz integral, arroz yamani o trigo burgol 👌";
      if (p.includes("salmon")) return "El salmon se puede reemplazar por atun fresco, caballa o merluza 💪";
      if (p.includes("pollo")) return "Podes reemplazar el pollo por pavo, cerdo magro o tofu firme 🍗";
      if (p.includes("palta")) return "Si no tenes palta, proba con ricota o queso crema light 🥑";
      if (p.includes("brocoli")) return "El brocoli se puede cambiar por coliflor, chauchas o zapallito verde 🥦";
      if (p.includes("lentejas")) return "Las lentejas se pueden reemplazar por porotos, garbanzos o arvejas 🫘";
      if (p.includes("arroz")) return "Podes usar arroz blanco comun (menos tiempo, 15 min) o fideos integrales 🍚";
      if (p.includes("aceite")) return "Podes usar aceite de girasol o canola para cocinar 🫒";
      if (p.includes("ricota")) return "La ricota se puede reemplazar por queso cottage o queso crema descremado 🧀";
      return "Para ese ingrediente, busca algo de textura y sabor similar. Que ingrediente especifico te falta? 😊";
    }
    if (p.includes("tiempo") || p.includes("listo") || p.includes("cocido") || p.includes("cuando")) {
      if (p.includes("pollo")) return "El pollo esta listo cuando al pincharlo el jugo sale claro. A la plancha: 4-5 min por lado. Al horno 200C: 35-45 min 🍗";
      if (p.includes("arroz")) return "Arroz blanco: 15-18 min tapado. Arroz integral: 35-40 min. Cuando aparecen agujeritos en la superficie, apagar y reposar 5 min 🍚";
      if (p.includes("quinoa")) return "La quinoa tarda 15 min. Lista cuando aparece el germen blanco alrededor del grano. Reposar 5 min tapada ✅";
      if (p.includes("lentejas")) return "Lentejas rojas: 15-20 min. Pardas o verdes: 25-30 min. Listas cuando se aplastan con el tenedor 🫘";
      if (p.includes("huevo")) return "Huevo duro: 10 min. Pasado por agua: 6 min. Revuelto: fuego bajo, retirar cuando casi cuajan 🥚";
      return "El punto justo es cuando cambia de color y se puede pinchar facilmente. Que alimento especifico? ⏱";
    }
    if (p.includes("dura") || p.includes("guardar") || p.includes("heladera") || p.includes("freezer")) {
      if (p.includes("pollo") || p.includes("carne")) return "La carne cocida dura 3-4 dias en heladera. En freezer hasta 3 meses 🧊";
      if (p.includes("arroz") || p.includes("quinoa") || p.includes("lentejas")) return "Cereales y legumbres cocidos duran 4-5 dias en heladera. Ideales para batch cooking 📦";
      if (p.includes("sopa") || p.includes("guiso")) return "Sopas y guisos duran 4-5 dias en heladera y se pueden freezar hasta 3 meses 🍲";
      return "Comidas cocidas duran 3-4 dias en heladera en recipiente hermetico. Enfriar antes de guardar ❄️";
    }
    if (p.includes("hola") || p.includes("gracias") || p === "ok" || p === "bien") return "Hola! Estoy aca para ayudarte con cualquier duda de cocina 👨‍🍳";
    return "Buena pregunta 👨‍🍳 Podes contarme mas sobre que receta o ingrediente tenes dudas y te ayudo mejor 😊";
  };
      ...
      <div style={{ position:"relative", background:C.white, borderRadius:"24px 24px 0 0", width:"100%", maxWidth:580, margin:"0 auto", maxHeight:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 -8px 40px rgba(59,111,212,0.2)" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"16px 16px 8px", display:"flex", flexDirection:"column", gap:12 }}>
          {msgs.map((m, i) => (
            <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}>
              ...
              <div style={{ maxWidth:"78%", background:m.role==="user"?`linear-gradient(135deg,${C.blue},${C.blueDk})`:C.gray1, color:m.role==="user"?C.white:C.text, borderRadius:m.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px", padding:"12px 14px", fontSize:14, lineHeight:1.6, whiteSpace:"pre-wrap" }}>{m.text}</div>
            </div>
          ))}
          {loading && (
          ...
          {msgs.length === 1 && (
            <div style={{ display:"flex", flexWrap:"wrap", gap:7, marginTop:4 }}>
              {["Puedo reemplazar el salmon?","Como se si el pollo esta listo?","Me falta quinoa, que uso?","Cuanto dura en la heladera?"].map((s, i) => (
                <button key={i} onClick={() => { setInput(s); setTimeout(() => inputRef.current?.focus(), 50); }} style={{ background:C.blueLt, color:C.blue, border:`1px solid ${C.bluePl}`, borderRadius:20, padding:"7px 13px", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>{s}</button>
              ))}
            </div>
          )}
          <div ref={bottomRef}/>
        ...
        <div style={{ padding:"12px 16px 20px", borderTop:`1px solid ${C.gray2}`, display:"flex", gap:10, alignItems:"flex-end" }}>
          <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if(e.key==="Enter" && !e.shiftKey) { e.preventDefault(); send(); }}} placeholder="Preguntale al chef..." rows={1}
            style={{ flex:1, padding:"12px 14px", borderRadius:14, border:`2px solid ${C.gray2}`, fontSize:14, color:C.text, resize:"none", lineHeight:1.5, background:C.gray1, fontFamily:"'DM Sans',sans-serif", outline:"none", maxHeight:80, overflowY:"auto" }}
            onFocus={e => e.target.style.borderColor=C.blue} onBlur={e => e.target.style.borderColor=C.gray2}/>
          <button onClick={send} disabled={!input.trim() || loading} style={{ width:44, height:44, borderRadius:14, border:"none", background:!input.trim()||loading?C.gray2:`linear-gradient(135deg,${C.blue},${C.blueDk})`, color:!input.trim()||loading?C.gray4:C.white, fontSize:18, cursor:!input.trim()||loading?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>↑</button>
        </div>
      </div>
  ...
  if (cookMode) return (
    <div style={{ position:"fixed",inset:0,background:C.dark,zIndex:3000,overflowY:"auto",padding:"0 0 60px" }}>
      ...
      <div style={{ padding:"8px 24px 0" }}>
        ...
        <button onClick={()=>{ setCookMode(false); onClose(); }} style={{ width:"100%",background:C.blue,color:C.white,border:"none",borderRadius:16,padding:"18px",fontSize:17,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginTop:8 }}>✓ Listo, termine de cocinar</button>
      </div>
    </div>
  );
  return (
    <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(10,15,30,0.7)",zIndex:2000,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(6px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:580,maxHeight:"90vh",overflowY:"auto",paddingBottom:40 }}>
        ...
        <div style={{ padding:"20px 24px 0" }}>
          ...
          <div style={{ fontSize:12,fontWeight:800,color:C.blue,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12 }}>Preparacion</div>
          ...
          <button onClick={()=>setCookMode(true)} style={{ marginTop:24,width:"100%",background:C.dark,color:C.white,border:"none",borderRadius:16,padding:"15px",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>🍳 Modo cocina - pantalla limpia</button>
          <button onClick={onClose} style={{ marginTop:10,width:"100%",background:`linear-gradient(135deg,${C.blue},${C.blueDk})`,color:C.white,border:"none",borderRadius:16,padding:"15px",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",boxShadow:sh.blue }}>✓ Listo, entendido</button>
        </div>
      </div>
    </div>
  );
...
function MainApp() {
  ...
  useEffect(() => {
    if (screen === "loading") {
      const msgs = ["Analizando tu objetivo...","Seleccionando recetas...","Calculando la lista...","Optimizando el presupuesto...","Listo!"];
      let i = 0;
    ...
    <div ref={scrollRef} style={{ minHeight:"100vh", background:C.white, fontFamily:"'DM Sans',sans-serif", overflowY:"auto", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"8px 28px 0", maxWidth:480, margin:"0 auto", width:"100%" }}>
        <div style={{ display:"flex", justifyContent:"center", marginBottom:4 }}>
          <img src="https://i.imgur.com/IYcv1Vp.jpeg" alt="Kooki" style={{ width:"60%", maxWidth:220, borderRadius:0, background:"transparent" }}/>
        </div>
        ...
        <p style={{ fontSize:15, color:C.sub, lineHeight:1.7, marginBottom:16, textAlign:"center", width:"100%" }}>Vos respondes. Kooki resuelve.</p>
        ...
        <div style={{ background:C.bg, borderRadius:20, padding:"24px 24px", marginBottom:28, border:`1px solid ${C.gray2}` }}>
          ...
          {[["✔️","Menu completo de lunes a domingo"],["✔️","Lista de compras organizada por seccion"],["✔️","Recetas paso a paso con modo cocina"],["✔️","Chef asistente con IA para tus dudas"]].map(([ic,txt],i) => (
        ...
        <button onClick={() => { setScreen("onboarding"); setStep(0); setAnswers({}); }} style={{ width:"100%", background:`linear-gradient(135deg,${C.blue},${C.blueDk})`, color:C.white, border:"none", borderRadius:18, padding:"20px", fontSize:18, fontWeight:800, cursor:"pointer", boxShadow:sh.blue, fontFamily:"'DM Sans',sans-serif", marginBottom:12 }}>
          📅 Armar mi semana →
        </button>
        <button onClick={() => { setHoyResult(null); setScreen("hoy"); }} style={{ width:"100%", background:C.white, color:C.blue, border:`2px solid ${C.blue}`, borderRadius:18, padding:"18px", fontSize:17, fontWeight:800, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginBottom:24 }}>
          🍳 ¿Que cocino hoy?
        </button>
  ...
  if (screen === "hoy") return (
    <div style={{ minHeight:"100vh", background:C.white, fontFamily:"'DM Sans',sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:"14px 20px", display:"flex", alignItems:"center", gap:14, borderBottom:`1px solid ${C.gray2}` }}>
        ...
        <div style={{ fontSize:17, fontWeight:800, color:C.dark }}>¿Que cocino hoy?</div>
      </div>
      {!hoyResult ? (
        <div style={{ flex:1, padding:"28px 24px", maxWidth:520, margin:"0 auto", width:"100%" }}>
          <div style={{ fontSize:28, fontWeight:800, color:C.dark, marginBottom:8, letterSpacing:"-0.5px" }}>Rapido y facil 🍳</div>
          <div style={{ fontSize:15, color:C.sub, marginBottom:32, lineHeight:1.55 }}>Te sugerimos recetas al instante.</div>
          <div style={{ marginBottom:32 }}>
            <div style={{ fontSize:15, fontWeight:700, color:C.dark, marginBottom:12 }}>¿Cuantas comidas necesitas?</div>
            <div style={{ display:"flex", gap:10 }}>
              {[["1","🍽️ Una sola comida"],["2","🌞🌙 Almuerzo y cena"]].map(([val,lbl]) => (
                <button key={val} onClick={() => setHoyCuantas(val)} style={{ flex:1, padding:"16px 12px", borderRadius:16, border:`2px solid ${hoyCuantas===val?C.blue:C.gray2}`, background:hoyCuantas===val?C.blueLt:C.white, fontSize:14, fontWeight:700, color:hoyCuantas===val?C.blueDk:C.dark, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.18s" }}>{lbl}</button>
              ))}
            </div>
          </div>
          <button onClick={() => setHoyResult(generarHoy(hoyCuantas))}
            style={{ width:"100%", background:`linear-gradient(135deg,${C.blue},${C.blueDk})`, color:C.white, border:"none", borderRadius:18, padding:"20px", fontSize:18, fontWeight:800, cursor:"pointer", boxShadow:sh.blue, fontFamily:"'DM Sans',sans-serif" }}>
            ✨ Sugerir receta
          </button>
        </div>
      ) : (
        <div style={{ flex:1, padding:"28px 24px", maxWidth:520, margin:"0 auto", width:"100%" }}>
          <div style={{ fontSize:24, fontWeight:800, color:C.dark, marginBottom:6, letterSpacing:"-0.5px" }}>Tu menu de hoy 🎉</div>
          <div style={{ fontSize:14, color:C.sub, marginBottom:24 }}>Recetas seleccionadas al azar para vos</div>
            ...
            {hoyResult.map((item, i) => {
              ...
              return (
                <div key={i} style={{ background:C.white, borderRadius:18, padding:"20px", boxShadow:sh.md }}>
                  <div style={{ fontSize:12, fontWeight:800, color:C.blue, textTransform:"uppercase", letterSpacing:"1px", marginBottom:8 }}>{item.tipo}</div>
                  ...
                  <button onClick={() => setReceta(item.nombre)} style={{ marginTop:14, width:"100%", background:C.blueLt, color:C.blue, border:"none", borderRadius:12, padding:"12px", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>Ver receta completa →</button>
                </div>
              );
            })}
          </div>
          <button onClick={() => setHoyResult(generarHoy(hoyCuantas))} style={{ width:"100%", background:C.white, color:C.blue, border:`2px solid ${C.blue}`, borderRadius:16, padding:"16px", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginBottom:12 }}>
            ↺ Otra sugerencia
          </button>
          <button onClick={() => setHoyResult(null)} style={{ width:"100%", background:C.gray1, color:C.sub, border:"none", borderRadius:16, padding:"14px", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
            ← Volver
          </button>
        </div>
      )}
        ...
        <div key={animKey} style={{ flex:1, padding:"24px 20px 32px", overflowY:"auto", maxWidth:520, margin:"0 auto", width:"100%", animation:"slideIn 0.3s ease" }}>
          ...
          {cur.type === "objetivo" && (
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {cur.options.map(opt => {
                ...
                return (
                  <button key={opt.value} onClick={toggle} style={{ padding:"16px 18px", borderRadius:16, cursor:"pointer", textAlign:"left", border:`2px solid ${sel?C.blue:C.gray2}`, background:sel?opt.color||C.blueLt:C.white, display:"flex", alignItems:"center", gap:14, transition:"all 0.18s", fontFamily:"'DM Sans',sans-serif", boxShadow:sel?`0 0 0 3px ${C.blue}22,${sh.md}`:sh.sm }}>
          ...
          {cur.type === "grid" && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {cur.options.map(opt => {
                ...
                return (
                  <button key={opt.value} onClick={toggle} style={{ padding:"18px 12px", borderRadius:16, cursor:"pointer", textAlign:"center", border:`2px solid ${sel?C.blue:C.gray2}`, background:sel?C.blueLt:C.white, display:"flex", flexDirection:"column", alignItems:"center", gap:8, transition:"all 0.18s", boxShadow:sel?`0 0 0 3px ${C.blue}22,${sh.md}`:sh.sm, fontFamily:"'DM Sans',sans-serif" }}>
          ...
          {(cur.type === "cards" || cur.type === "nivel") && (
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {cur.options.map(opt => {
                ...
                return (
                  <button key={opt.value} onClick={() => setAnswers(a=>({...a,[cur.id]:opt.value}))} style={{ padding:"16px 18px", borderRadius:16, cursor:"pointer", textAlign:"left", border:`2px solid ${sel?C.blue:C.gray2}`, background:sel?opt.accent||C.blueLt:C.white, display:"flex", alignItems:"center", gap:14, transition:"all 0.18s", fontFamily:"'DM Sans',sans-serif", boxShadow:sel?`0 0 0 3px ${C.blue}22,${sh.md}`:sh.sm }}>
                    <div style={{ width:48, height:48, borderRadius:14, background:sel?C.blue:C.gray1, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, flexShrink:0 }}>{opt.emoji}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:16, fontWeight:700, color:sel?C.blueDk:C.dark }}>{opt.label}</div>
                      {opt.desc && <div style={{ fontSize:13, color:C.sub, marginTop:2 }}>{opt.desc}</div>}
          ...
          {cur.type === "extras" && (
            <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
              {[{id:"ingredientes",label:"Que tenes en casa ahora?",ph:"Ej: arroz, huevos, pollo, tomates...",e:"🧺"},{id:"no_gusta",label:"Algo que no comas?",ph:"Ej: cebolla cruda, mariscos...",e:"🚫"}].map(f => (
                <div key={f.id}>
          ...
          <button onClick={handleNext} disabled={!canNext()} style={{ width:"100%", marginTop:28, padding:"18px", borderRadius:16, border:"none", fontSize:17, fontWeight:800, background:canNext()?`linear-gradient(135deg,${C.blue},${C.blueDk})`:C.gray2, color:canNext()?C.white:C.gray4, cursor:canNext()?"pointer":"not-allowed", boxShadow:canNext()?sh.blue:"none", transition:"all 0.2s", fontFamily:"'DM Sans',sans-serif" }}>
  ...
  if (screen === "loading") {
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.white, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32, fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <h2 style={{ fontSize:26, fontWeight:800, color:C.dark, marginBottom:10, textAlign:"center", letterSpacing:"-0.5px" }}>Kooki esta trabajando...</h2>
      ...
      <div ref={scrollRef} style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", overflowY:"auto" }}>
        ...
        <div style={{ background:`linear-gradient(140deg,${C.blue},${C.blueDk})`, padding:"22px 22px 36px", position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:22, position:"relative" }}>
            ...
            <button onClick={resetAll} style={{ background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.25)", color:C.white, borderRadius:12, padding:"8px 16px", fontSize:13, cursor:"pointer", fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>↺ Nuevo menu</button>
          </div>
          <div style={{ position:"relative" }}>
            ...
            <div style={{ fontSize:24, fontWeight:800, color:C.white, marginBottom:8, letterSpacing:"-0.5px" }}>Tu plan esta listo 🎉</div>
                      ...
                      <div key={k} style={{ marginBottom:k==="alm"?12:0 }}>
                        <div style={{ display:"flex", alignItems:"flex-start" }}>
                          ...
                          <div style={{ display:"flex", gap:6, flexShrink:0, marginLeft:10, paddingTop:2 }}>
                            ...
                            <button onClick={()=>handleCambiar(d.dia,k)} style={{ background:C.gray1, border:`1px solid ${C.gray2}`, borderRadius:8, padding:"5px 9px", fontSize:13, color:cambiando===`${d.dia}-${k}`?C.blue:C.gray4, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
                              {cambiando===`${d.dia}-${k}`?"⏳":"🔄"}
                            </button>
                          </div>
                        </div>
        ...
        {showChefFab && (
          <div style={{ position:"fixed", bottom:28, right:22, zIndex:100 }}>
            ...
            {chefPulse && (
              <div style={{ position:"absolute", bottom:"110%", right:0, background:C.dark, color:C.white, borderRadius:12, padding:"8px 12px", fontSize:12, fontWeight:700, whiteSpace:"nowrap", boxShadow:sh.md, marginBottom:4 }}>
            ...
            <button onClick={() => { setChefOpen(true); setChefPulse(false); }} style={{ width:60, height:60, borderRadius:"50%", border:"none", background:`linear-gradient(135deg,${C.blue},${C.blueDk})`, color:C.white, fontSize:26, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:sh.blue, fontFamily:"'DM Sans',sans-serif", animation:chefPulse?"chefPulse 1.5s ease infinite":"chefBounce 2s ease infinite" }}>
              👨‍🍳
            </button>
          </div>
        )}
